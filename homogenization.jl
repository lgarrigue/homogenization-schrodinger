...
function solve_lobpcg(H,l,p,maxiter=100,tol=1e-5) # finds the first l eigenmodes of the action H
	X0 = randn(ComplexF64,p.Nk,l)
	λs = []; φs = []; cv = true
	if p.Nk > 50 && !(p.N/p.D == 1.)
		@time res = lobpcg_hyper(H,X0,p.k_list_squared; maxiter=maxiter, tol=tol)
		px("LOBPCG Converged ? ",res.converged," | Iterations= ",res.iterations)
		φs = [res.X[:,i] for i in 1:size(res.X,2)]
		λs = res.λ
		cv = res.converged
	else
		matH = Hermitian(linmap2mat(H,p.Nk))
		sol = eigen(matH, 1:l)
...
function eigenmodes(A,Nε,l,p,maxiter,tol,computes=true)
	if computes
		qN = sqrt(Nε)
		(λ,ϕ,conv_ε) = solve_lobpcg(A,l,p,maxiter,tol)
		λ = λ[l]*Nε
		u_fourier = qN*ϕ[l]
...
function study(todo)
	...
	if todo=="plot_curves"
		...
		if todo=="kinpot"
			...
			nmin = log(Dεs[end]/Nεs[end])/log(10)
			len_others = 10
			Dεs = vcat(Dεs,Int.(floor.(10 .^(range(nmin,stop=1.2,length=len_others))))); Nεs = vcat(Nεs,fill(1,len_others))
			# Dεs = Int.(floor.(10 .^(range(nmin,stop=2.5,length=len_others)))); Nεs = fill(1,len_others)
		end
		εs = Nεs ./ Dεs
		px("l= ",l,"\n Dεs = ",Dεs,"\n Nεs = ",Nεs,"\n εs = ",εs)
